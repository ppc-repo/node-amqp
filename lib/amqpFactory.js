...
function rpc(name, routingkey,  msg, options, queueName) {
    var logger = process.logger || console;
    if(!name) throw new Error('Name of connection require')
    var connection = getConnection(name);
    if(!connection) throw new Error('No Connection found with Name ' + name);
    connection.queue(queueName, {exclusive: true}, function(queue) {
        connection.publish(routingkey, msg, clientRequest.getOptions({replyTo: queue.name})) ;
    ...
    //var queue = queues[0];
    //queue.subscribe(options, function(payload, headers, deliveryInfo, msg) {
