...
var exchanges = {};
//TODO the queues obj does not hold the queue opject but the config need to refactor
var queues = {};
            ...
            case true :
                if(item.exchange ===  exchange.name) {
                    ...
                    var queue = connection.queue(item.name, item.options, function(q) {
                        ...
                        if(item.callback) {
                            var subOptions = item.subscribeOptions ||  { ack: true, prefetchCount : 1 };
                            q.subscribe(subOptions,  item.callback);
                        }
                    });
...
function subscribe(queueName, options, cb) {
    var logger = process.logger || console;
    ...
    var queue =   connection.queue(queueName, options, function(q) {
